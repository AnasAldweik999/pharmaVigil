...
public class UserEmailNotTakenByOthersValidator implements Validator<User> {

    private final UserRepository repository;

    public UserEmailNotTakenByOthersValidator(UserRepository repository) {
    ...
    public Set<SystemViolation> validate(User user) {
        ...
        if (repository.existsByEmailIgnoreCaseAndIdNot(user.getEmail(), user.getId())) {
