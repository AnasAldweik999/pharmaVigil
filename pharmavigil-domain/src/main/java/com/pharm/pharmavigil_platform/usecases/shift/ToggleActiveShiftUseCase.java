...
@Slf4j
public class ToggleActiveShiftUseCase {

    private final ValidatorChain<Shift> validators;
    private final ShiftRepository repository;
    private final IdentityProvider identityProvider;

    public ToggleActiveShiftUseCase(ValidatorChain<Shift> validators, ShiftRepository repository, IdentityProvider identityProvider) {
    ...
    public Shift execute(Shift shift) {
        log.info("Toggling active status for shift with id '{}'", shift.getId());
        ...
        log.info("Shift '{}' active status changed to '{}'", shift.getId(), saved.isActive());
        return saved;
    }
}
