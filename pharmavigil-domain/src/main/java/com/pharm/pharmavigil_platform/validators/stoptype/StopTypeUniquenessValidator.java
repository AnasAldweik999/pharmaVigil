...
public class StopTypeUniquenessValidator implements Validator<StopType> {

    private final StopTypeRepository repository;

    public StopTypeUniquenessValidator(StopTypeRepository repository) {
    ...
    public Set<SystemViolation> validate(StopType stopType) {
        ...
        if (repository.existsByNameIgnoreCase(stopType.getName())) {
